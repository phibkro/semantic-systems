...
| Open semantic system                | Treat maintained state as warranted knowledge and outward effects as protocols    | The boundary and returned observation strength must be explicit        |
| Artifact/effect separation          | Derive values before requesting persistence or delivery                           | Request issuance is not the worldly consequence                        |
...
| Bounded autonomy                    | Combine persistent components with bounded reactions and task scopes              | Queue, retry, fan-out, progress, and recovery policies remain explicit |
| Orthogonal system graphs            | Separate ownership, supervision, communication, tasks, derivation, and deployment | One convenient hierarchy must not silently stand in for another        |
