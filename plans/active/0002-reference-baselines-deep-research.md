...
- Live repository policy remains stricter: at most two delegated workers and
  depth two.
...
- Fable 5 workflow lead: pending launch in Herdr tab
  `semantic-reference-fable`.
- Workflow Sonnet/Opus children: pending Fable routing; one active at a time.
...
- Sequential admission preserves correctness while increasing elapsed time.
...
- “Hundreds of agents” means total resumable tasks across batches, not hundreds
  live concurrently.
- The pilot deliberately uses one workflow child beside Fable despite Claude
  Code supporting sixteen concurrent agents.
