...
- `refs.bib` is the canonical bibliography for papers, books, reports,
  authorship, venues, and persistent publication identifiers. Do not duplicate
  that metadata into `sources.toml`.
- `sources.lock.json` is generated by `semrefs lock` and records the exact
  commit, tree, object format, catalog digest, every declared license artifact's
  bytes and metadata, retrieval time, acquisition kind, and resolved ref used
  by an accepted custody observation.
...
`plans/completed/0004-reference-source-custody.md` and its canonical feature lifecycle
...
The `local.lang-bang` lock entry records a local-sibling acquisition, so its
status deliberately shows a weaker visible assumption rather than claiming
canonical-origin verification.

`refs.bib` contains accepted primary papers and reports whose bodies were read
directly. Papers named only in source questions remain discovery hints, not
accepted bibliography records.
