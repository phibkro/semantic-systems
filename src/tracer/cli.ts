...
import { evidencePassedCases, evidenceTotalCases } from "./evidence.ts";
  ...
  Effect.gen(function* () {
    ...
    yield* Console.log(
      resolution.status === "selected"
        ? `Selected: ${resolution.selectedRealization}`
...
const parse = (
...
): { readonly root: string; readonly policy: string } | undefined => {
  if (arguments_[0] !== "demo" || arguments_[1] === undefined) return undefined;
  ...
  return { root: arguments_[1], policy };
};
...
export const runTracerCli = (
  ...
  if (command === undefined) {
    return Console.error("usage: semantic-tracer demo ROOT [--policy POLICY]").pipe(Effect.as(2));
  }
  return Effect.gen(function* () {
    const path = yield* Path.Path;
    const result = yield* runDemo(path.resolve(command.root), command.policy);
    yield* printReport(result);
    return result.resolution.status === "selected" &&
