...
class ProjectGraph:
    ...
    def by_kind(self, kind: str) -> tuple[Entity, ...]:
        return tuple(entity for _, entity in sorted(self.entities.items()) if entity.kind == kind)

    def outgoing(self, entity_id: str, kinds: set[str] | None = None) -> tuple[Relation, ...]:
        return tuple(
            relation
            for relation in self.relations
            if relation.source_id == entity_id and (kinds is None or relation.kind in kinds)
        )
    ...
    def incoming(self, entity_id: str, kinds: set[str] | None = None) -> tuple[Relation, ...]:
        return tuple(
            relation
            for relation in self.relations
            if relation.target_id == entity_id and (kinds is None or relation.kind in kinds)
        )
