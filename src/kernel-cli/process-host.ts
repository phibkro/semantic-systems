...
import { defaultKernelJsonRawBounds } from "../kernel-json/index.ts";
import { KernelCliHostError, type KernelCliHost } from "./cli.ts";

const maximumInputBytes = defaultKernelJsonRawBounds.maximumBytes + 1;
...
): KernelCliHostError => new KernelCliHostError({ operation });

const readFilePrefix = (path: string): Effect.Effect<Uint8Array, KernelCliHostError> =>
...
const readStdinPrefix = (): Effect.Effect<Uint8Array, KernelCliHostError> =>
  Effect.tryPromise({
    try: async () => {
      const bytes = new Uint8Array(maximumInputBytes);
      let offset = 0;
...
): Effect.Effect<void, KernelCliHostError> =>
  Effect.tryPromise({
    try: () =>
...
export const makeProcessKernelCliHost = (): KernelCliHost => ({
