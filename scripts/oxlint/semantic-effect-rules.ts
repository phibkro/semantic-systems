  ...
  create: function* () {
    ...
    return yield* Visitor.filter(
      ...
      Visitor.merge(
        Visitor.on("CallExpression", (node) => {
          let memberRoot:
            | (Parameters<typeof ctx.sourceCode.isGlobalReference>[0] & {
                readonly name: string;
              })
            | undefined;
          ...
          const ambientMemberRoot = memberRoot !== undefined && isAmbientReference(memberRoot);
          ...
          const ambientTimer =
            (node.callee.type === "Identifier" &&
              ...
              isAmbientReference(node.callee)) ||
            (ambientMemberRoot &&
